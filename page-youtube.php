...
<?php get_header('page') ?>
 ...
 <!-- カスタム投稿タイプ post_works の全記事へのリンク -->
    <li class="inView hover">
        <a href="<?php echo esc_url(home_url('youtube')); ?>" class="works__category--list works__category--item">すべて</a>
    </li>

    <!-- カテゴリー '宿泊キャンプ' (slug: camp) を含む記事へのリンク -->
    <li class="inView hover">
        <a href="<?php echo esc_url(get_term_link('camp', 'category')); ?>" class="works__category--list works__category--item">宿泊キャンプ</a>
    </li>

    <!-- カテゴリー '施設紹介' (slug: introduction) を含む記事へのリンク -->
    <li class="inView hover">
        <a href="<?php echo esc_url(get_term_link('introduction', 'category')); ?>" class="works__category--list works__category--item">施設紹介</a>
    </li>

    <!-- カテゴリー 'その他' (slug: etc) を含む記事へのリンク -->
    <li class="inView hover">
        <a href="<?php echo esc_url(get_term_link('etc', 'category')); ?>" class="works__category--list works__category--item">その他</a>
    </li>
                    </ul>
                </div>
    ...
    while ($youtube_query->have_posts()) : $youtube_query->the_post(); ?>
            ...
            <a href="<?php the_permalink(); ?>" class="youtube__contents inView hover">
                <div class="youtube__contents--header">
                    ...
                    <?php else : ?>
                        <!-- サムネイルがない場合の代替画像 -->
                        <img src="<?php echo esc_url(get_template_directory_uri() . '/image/no-image/no-image.webp'); ?>" alt="No Image" width="528" height="297" class="youtube__contents--thumbnail">
                    <?php endif; ?>
                </div>
